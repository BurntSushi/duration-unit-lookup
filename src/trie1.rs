// This is the trie I originally wrote for Jiff, and after benchmarking, I
// surmised that it was not as fast as I hoped.
...
> {
    ...
    /// The length of the longest needle in this trie, in bytes.
    max: usize,
}
    ...
    > Trie<NODE_CAPACITY, ALPHABET_LEN, V>
{
    ...
    pub(crate) fn find<'h>(
        ...
        let mut node_id = Self::ROOT_ID;
        let mut found =
            self.nodes[node_id as usize].value.map(|v| (v, haystack));
        ...
        while let Some(&byte) = it.next() {
            ...
            if let Some(value) = self.nodes[node_id as usize].value {
                found = Some((value, it.as_slice()));
            }
        }
        found
    }

    /// Like `find`, but returns the remainder as an offset into `haystack`.
    ///
    /// This is only used in tests because it's easier to write the offset.
    #[cfg(test)]
    pub(crate) fn find_offset(&self, haystack: &[u8]) -> Option<(V, usize)> {
        let (v, slice) = self.find(haystack)?;
        Some((v, haystack.len() - slice.len()))
    }
...
mod tests {
    ...
    fn empty() {
        ...
        assert_eq!(EMPTY_TRIE.find_offset(b""), Some(('z', 0)));
    }
    ...
    fn fubar() {
        ...
        assert_eq!(FOO_TRIE.find_offset(b""), None);
        assert_eq!(FOO_TRIE.find_offset(b"fo"), None);
        assert_eq!(FOO_TRIE.find_offset(b"foo"), Some(('a', 3)));
        assert_eq!(FOO_TRIE.find_offset(b"fooquux"), Some(('a', 3)));
    }
    ...
    fn aaa() {
        ...
        assert_eq!(AAA_TRIE.find_offset(b""), None);
        assert_eq!(AAA_TRIE.find_offset(b"a"), Some(('a', 1)));
        assert_eq!(AAA_TRIE.find_offset(b"aa"), Some(('b', 2)));
        assert_eq!(AAA_TRIE.find_offset(b"aaa"), Some(('c', 3)));
        assert_eq!(AAA_TRIE.find_offset(b"aaaa"), Some(('c', 3)));
    }
}
