// This evolved from trie3, and represents a major change to how its state
// machine is laid out in memory. Now our match values are separate from our
// transition table. And our transition table is no longer nested arrays, but
// just one big array. And we pre-multiply our node IDs.
//
// This trie overall uses a fair bit more space. For two reasons. First is
// because our node ID is now a `u16` instead of a `u8`. This is because of the
// pre-multiplied optimization (where a node index of 2 is mapped to a node
// id of `2 * ALPHABET_LEN`). This saves a multiply in the search loop at the
// cost needing a bigger primitive type to store the IDs. The second reason is
// that our `matches` array is the same size as our transition table, which is
// clearly wasteful. We could shrink this a bit by converting the match node ID
// back to its node index, but I didn't want to spend the effort. (You naively
// think this requires a `div` per `find` that finds a match. To avoid that,
// you need to make the alphabet length a power of 2. Then you can do a shift
// instead of a div. Of course, this means wasting a bit more space and dealing
// with the power-of-2 logic.)
//
// Overall, this trie uses about 3x the space of the previous 3 tries. It
// comes in at about 6.3KB... which feels a little big. Hell, the 2.3KB used
// by the previous tries felt a little big! Some other things to try here would
// be to see just how much time we're saving by pre-multiplying the IDs. Maybe
// it's insignificant. But I wanted to go a different direction with trie5, so
// I abandoned this particular trie4.
    ...
    > Trie<TRANSITION_CAPACITY, NEEDLE_LEN, ALPHABET_LEN, V>
{
    ...
    pub(crate) fn find<'h>(&self, haystack: &'h [u8]) -> Option<(V, usize)> {
        ...
        let mut found = if node_id <= self.max_match_node_id {
            Some((node_id, 0))
        } else {
            None
        };
        for (i, &byte) in haystack.iter().enumerate() {
            ...
            if node_id <= self.max_match_node_id {
                if node_id == Self::FAIL_ID {
                    break;
                }
        ...
        found.map(|(node_id, offset)| (self.matches[node_id as usize], offset))
    }
}
