// This is almost identical to trie1.rs, but instead of returning a slice for
// the remaining input on `find`, it returns the end position of the match.
// Benchmarks suggest it is just about identical in performance to trie1.
    ...
    > Trie<NODE_CAPACITY, ALPHABET_LEN, V>
{
    ...
    pub(crate) fn find<'h>(&self, haystack: &'h [u8]) -> Option<(V, usize)> {
        let mut node_id = Self::ROOT_ID;
        let mut found = self.nodes[node_id as usize].value.map(|v| (v, 0));
